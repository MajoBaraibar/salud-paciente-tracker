-- Actualizar los tipos de citas permitidos según el rol
-- Agregar más tipos específicos para médicos, enfermeras y administración

-- Para médicos: consulta, interconsulta, junta_medica, seguimiento
-- Para enfermeras: reunion_staff, capacitacion, supervision, procedimiento_enfermeria  
-- Para administración: reunion_familia, auditoria, capacitacion_staff, evaluacion
...
-- Insertar citas de ejemplo con los nuevos tipos
...
-- Citas para enfermeras
(
  NULL, -- Sin paciente específico para reunión de staff
...
(
  NULL, -- Sin paciente específico para capacitación
  (SELECT id FROM profiles WHERE role = 'enfermera' LIMIT 1),
