...
BEGIN
  ...
  SELECT 
    ...
    0 as consultas_realizadas, -- Se actualizará con triggers
    0 as emergencias_atendidas,
    LEAST(100, (COUNT(*) * 100.0 / 50)) as ocupacion_porcentaje -- asumiendo capacidad de 50
...
-- Insertar datos de ejemplo para métricas
...
SELECT 
  cs.id,
  CASE 
    WHEN random() < 0.3 THEN 'critica'
    WHEN random() < 0.6 THEN 'importante' 
    ELSE 'informativa'
  END,
  CASE 
    WHEN random() < 0.5 THEN 'Medicamento próximo a vencer'
    ELSE 'Revisión médica pendiente'
  END,
  'Descripción de la alerta del sistema',
  CASE 
